...
endButton.addEventListener("click", async(e) => {
  for(let i = 1; i <= NUMBER_OF_PLAYERS; i++){
    ...
    await gameScoreBoard.saveGameScore(playerName,scores[i-1]);
    scores[i-1] = 0;
  }
  const top10ScoreBoard = document.getElementById('top-10-score-board');
  await topWordAndGameScoreBoard.render(top10ScoreBoard);
});