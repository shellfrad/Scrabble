...
app.get('/highestWordScores', async (req, res) => {
  ...
  topScores.forEach(score => {
    scoresArray.push({ name: score.name, word: score.word, score: score.score });
  });
  res.status(200).json({scores: scoresArray});
});
...
app.get('/highestGameScores', async (req, res) => {
 ...
 const scoresArray = [];
  topGameScores.forEach(score => {
    scoresArray.push({ name: score.name, score: score.score });
 });
  res.status(200).json({scores: scoresArray});
  //res.status(200).json(topGameScores);
});
